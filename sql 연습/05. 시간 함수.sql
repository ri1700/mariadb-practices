...
-- now() -> 쿼리가 실행하는 시점의 시간 vs sysdate() -> 쿼리가 실행되는 중에 시간
select now(), sysdate()
	from dual;
...
-- 예제01: 근무 개월
-- 포맷팅: YYMM, YYYYMM
select first_name, hire_date, period_diff(date_format(curdate(), '%y%m'), date_format(hire_date, '%y%m')) as '근무개월'
	from employees;


-- date_add(adddate), date_sub(subdate)
-- 예제02: 각 사원의 근속 연수가 5년이 되는 날에 휴가를 보내준다면 각 사원의 5년 근속 휴가 날짜
